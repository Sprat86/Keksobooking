...
// Create DOM element "notice"(map card):
    ...
    window.createCardArray = function (mapEl, publicationCreatingElement){ // Задаем функции параметр "publicationCreatingElement" и внутри функции работаем с этим параметром.
...
/** Функция преобразования типа жилья (функция перевода на русский язык): */
    ...
    window.createCard = function (mapEl, avatars) {
        for (let i = 0; i < avatars.length; i++) {
            let publicationBlock = document.createElement("article");
            publicationBlock.setAttribute('class', 'map__card popup');

            let popupAvatar = document.createElement('img');
            popupAvatar.setAttribute('src', avatars[i].author.avatar);
            popupAvatar.setAttribute('class', 'popup__avatar');
            popupAvatar.setAttribute("height", "70");
            popupAvatar.setAttribute("width", "70");
            publicationBlock.appendChild(popupAvatar);
            ...
            let title = document.createElement('h3');
            let h3 = document.createTextNode(avatars[i].offer.title);
            title.appendChild(h3);
            publicationBlock.appendChild(title);

            let address = document.createElement('p');
            let pAddress = document.createTextNode(avatars[i].offer.address);
            address.appendChild(pAddress);
            publicationBlock.appendChild(address);

            let price = document.createElement('p');
            let pPrice = document.createTextNode(avatars[i].offer.price + ' ' + RUBLE + '/ночь');
            price.setAttribute('class', 'popup_price');
            price.appendChild(pPrice);
            publicationBlock.appendChild(price);

            let type = document.createElement('h4');
            let h4 = document.createTextNode(getTypeTranslation(avatars[i].offer.type));
            type.appendChild(h4);
            publicationBlock.appendChild(type);

            let roomsGuest = document.createElement('p');
            let pRoomsGuest = document.createTextNode(avatars[i].offer.rooms + ' комнаты для ' + avatars[i].offer.guests + ' гостей');
            roomsGuest.appendChild(pRoomsGuest);
            publicationBlock.appendChild(roomsGuest);

            let checkInCheckOut = document.createElement('p');
            let pCheckInCheckOut = document.createTextNode('Заезд после ' + avatars[i].offer.checkin + ' , выезд после ' + avatars[i].offer.checkout);
            checkInCheckOut.appendChild(pCheckInCheckOut);
            publicationBlock.appendChild(checkInCheckOut);
            ...
            let features = document.createElement('ul');
            features.setAttribute('class', 'popup__features');
            for (let i = 0; i < avatars[i].offer.features.length; i++) {
                let li = document.createElement('li');
                li.setAttribute('class', 'feature feature--' + avatars[i].offer.features[i]);
                features.appendChild(li);
                publicationBlock.appendChild(features)
            }
            ...
            let description = document.createElement('p');
            let pDescription = document.createTextNode(avatars[i].offer.description);
            description.appendChild(pDescription);
            publicationBlock.appendChild(description);
            ...
            let photos = Array.from(avatars[i].offer.photos); /** Cоздаём новый экземпляр Array из массивоподобного объекта "avatars" (что равно xhr.response[i].offer.photos).*/
            ...
            mapEl.appendChild(publicationBlock);
            console.log(avatars[i].offer.type)
        }
    };
